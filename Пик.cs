...
namespace d1
{
	public partial class Пик : Form
	{
		...
		public Пик()
		{
			...
			{
				if (label.Text != "Атака" && label.Text != "Защита" && label.Text != "Танки" && label.Text != "Поддержка")
				{
					label.Visible = false;
				}
			}
			...
			{
				pictureBox.ImageLocation =  "image/" + Convert.ToString(pic) + ".png";
				pic++;
			}
		}
			...
			for (int i = 0; i < 28; i++)
			{
				if (pick[i])
				{
					...
					if (flag == 3)
					{
						...
						button1.Text = "Посмотреть результаты";
					}
			...
			if (Convert.ToString(command.ExecuteScalar()) == "1")                               // НЕ стерай, долбоеб, это ВАЖНО!
			{
				foreach (Label label in panel1.Controls.OfType<Label>())
				{
					...
					if (label.Text != "Атака" && label.Text != "Защита" && label.Text != "Танки" && label.Text != "Поддержка")
					{
						...
						finalpriority = (finalpriority * 100) / 28;
						...
						{
							label.Text = Convert.ToString(finalpriority) + "%";
						}
		...
		{
			MessageBox.Show(" Герои атаки это персонажи, которые специализируются на захвате объектов, \nа также продвижении груза и прочих агрессивных атакующих действиях. \nКак правило, вносят большую долю урона.");
		}

		private void linkLabel3_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			MessageBox.Show(" Герои защиты, как правило, имеют повышенную выживаемость, относительно атакующих персонажей, хотя и не дотягивают до танков. \nОбычно обладают меньшей мобильностью, нежели герои атаки и рассчитаны на совершение оборонительных действий из-за укрытия или с точки.");
		}

		private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			MessageBox.Show(" Класс, обладающий всем необходимым для длительного выживания на точке или рядом с грузом, таким как щиты, большое количество хп и т.д. , \nно в качестве платы за самую высокую в игре выживаемость, танки жертвуют своим уроном, который, как правило, значительно уступает героям атаки и обороны.");
		}

		private void linkLabel2_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
		{
			MessageBox.Show(" Поддержка — роль, к которой относятся герои, умеющие лечить или усиливать союзников. \nНо поддержка, которую оказывают герои этой роли, не всегда значит только исцеление или усиление противников. \nТак, Дзенъятта может вешать на врагов ослабляющие их сферы, а Симметра может контролировать точки на карте, размещая защитные турели. \nОбычно герои поддержки обладают небольшим запасом здоровья, сильно зависят от своих союзников и редко побеждают в схватках один на один. ");
		}
	}
}
